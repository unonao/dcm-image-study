...
export function Get_Dicom_Image(path,dcm_num,start,length){
    //path: /Explanation/Head/...(データパス) dcm_num: 601_(dicomの頭番号) length: 25(全データ数)
    const pathname= path;
    var img_list = []
    var array = []
    for(let i = start-1;i<length;i++){
    let fileName = dcm_num+String(i+1)+'.dcm';
